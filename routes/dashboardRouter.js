...
const { encryptMessage, uniqueId } = require("../utils/encryption.js");
const { sendSms } = require("../utils/sendSms.js");

router
  .post("/saveQuiz", authRequired, (req, res) => {
    const data = req.body.finalQnA;
    const quizId = uniqueId();
  ...
  .get("/quizbatches", authRequired, async (req, res) => {
    ...
    await quizDocRef.get().then((querySnapshot) => {
      querySnapshot.forEach((doc, index) =>
        batches.push({ value: doc.data().name, text: doc.data().name })
      );
    });
    res.send(batches);
  })
  ...
  .get("/editInfo/:id", authRequired, async (req, res) => {
    ...
    querySnapshot = await quizDocRef
      .collection("quiz")
      .doc(req.params.id)
      .get();
    res.send({
